...
describe("Simple cup generator test", () => {
  it("Fail to generate due to unsupported generator type", async () => {
    ...
    expect(response).toEqual({
      data: [],
      errors: [{ message: "Unsupported generator type", status: 422 }],
    });
  });
  ...
  it("Fail to generate due to invalid team names", async () => {
    ...
    expect(response).toEqual({
      data: [],
      errors: [{ message: "Invalid team names", status: 422 }],
    });
  });
  ...
  it("Generate a new competition with 4 teams", async () => {
    const teams = ["Porto", "Benfica", "Braga", "Sporting"];
  ...
  it("Generate a new complete competition with 16 teams", async () => {
    ...
    const response = generator(teams, {
      completeCup: true,
      type: "simple-cup",
    });
